...
namespace Module1.Helpers
{
    public class SingleViewCodeUpdateHelper
    {
        ...
        public string GetTheViewPath()
        {
            ...
            var singleViewCodePath = System.IO.Path.Combine(contentRootPath, "Areas", "Module1", "Pages", "SingleViewCode.cshtml");
        ...
        public MessageResult UpdateTheView(string singleViewCodePath)
        {
            ...
            {
                UpdateCount++;
                var update = $"    <p>{UpdateCount} => last_update_at: {DateTimeOffset.Now}</p>";
